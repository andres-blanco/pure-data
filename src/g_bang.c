...
void bng_draw_new(t_bng *x, t_glist *glist)
{
    ...
    sys_vgui(".x%lx.c create rectangle %d %d %d %d -width %d -outline %s -fill #%6.6x -tags %lxBASE\n",
             canvas, xpos, ypos,
             xpos + x->x_gui.x_w, ypos + x->x_gui.x_h,
             stroke_width,
             STYLE_IEM_BORDER_COLOR,
             x->x_gui.x_bcol, x);
    sys_vgui(".x%lx.c create oval %d %d %d %d -width %d -outline %s -fill #%6.6x -tags %lxBUT\n",
    ...
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -outline %s -tags [list %lxIN%d inlet]\n",
             canvas, xpos, ypos,
             xpos + IOWIDTH, ypos + xlet_height(x), STYLE_CONTROL_XLET_COLOR, x, 0);
}

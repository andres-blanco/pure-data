...
static inline int xlet_height(t_vradio* x) {
    return 1;
}
...
{
    t_canvas *canvas=glist_getcanvas(glist);
    int n=x->x_number, i, dy = x->x_gui.x_h, s4 = dy/4 - 2*(IEMGUI_ZOOM(x)-1);
    ...
    for(i=0; i<n; i++)
    {
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -width %d -outline %s -fill #%06x -tags %lxBASE%d\n",
                 canvas, xx11, yy11, xx12, yy12, STYLE_BORDER_WIDTH, STYLE_IEM_BORDER_COLOR,
                 x->x_gui.x_bcol, x, i);
    ...
    if(!x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -outline %s -tags [list %lxOUT%d outlet]\n",
             canvas, xx11, yy11 - xlet_height(x), xx11 + IOWIDTH, yy11, STYLE_CONTROL_XLET_COLOR, x, 0);
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -outline %s -tags [list %lxIN%d inlet]\n",
             canvas, xx11, yy11b, xx11 + IOWIDTH, yy11b + xlet_height(x), STYLE_CONTROL_XLET_COLOR, x, 0);
}
...
void vradio_draw_move(t_vradio *x, t_glist *glist)
{
    ...
    if(!x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c coords %lxOUT%d %d %d %d %d\n",
             canvas, x, 0, xx11, yy11 - xlet_height(x), xx11 + IOWIDTH, yy11);
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c coords %lxIN%d %d %d %d %d\n",
             canvas, x, 0, xx11, yy11b, xx11 + IOWIDTH, yy11b + xlet_height(x));
}
...
void vradio_draw_io(t_vradio* x, t_glist* glist, int old_snd_rcv_flags)
{
    ...
    if((old_snd_rcv_flags & IEM_GUI_OLD_SND_FLAG) && !x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill %s -tags %lxOUT%d\n",
                 ...
                 xpos+ IOWIDTH,
                 ypos+(x->x_number*x->x_gui.x_h), STYLE_IEM_BORDER_COLOR, x, 0);
    if(!(old_snd_rcv_flags & IEM_GUI_OLD_SND_FLAG) && x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c delete %lxOUT%d\n", canvas, x, 0);

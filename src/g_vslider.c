...
#define SLIDER_KNOB_X   1

static void vslider_draw_new(t_vslider *x, t_glist *glist)
{
    int xpos=text_xpix(&x->x_gui.x_obj, glist);
    int ypos=text_ypix(&x->x_gui.x_obj, glist);
    int r = ypos + x->x_gui.x_h - ((x->x_val + 50)/100) * IEMGUI_ZOOM(x);
    t_canvas *canvas=glist_getcanvas(glist);

    sys_vgui(".x%lx.c create rectangle %d %d %d %d -width %d -outline %s -fill #%06x -tags %lxBASE\n",
             canvas, xpos, ypos + VSLIDER_EXTRA_Y,
             xpos + x->x_gui.x_w, ypos + x->x_gui.x_h + VSLIDER_EXTRA_H,
             STYLE_BORDER_WIDTH, STYLE_IEM_BORDER_COLOR,
             x->x_gui.x_bcol, x);
    ...
    if(!x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxOUT%d outlet]\n",
             ...
             xpos + STYLE_XLET_WIDTH*IEMGUI_ZOOM(x), ypos + x->x_gui.x_h + VSLIDER_EXTRA_H,
             x, 0);
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxIN%d inlet]\n",
             canvas,
             xpos, ypos + VSLIDER_EXTRA_Y,
             xpos + (STYLE_XLET_WIDTH*IEMGUI_ZOOM(x)), ypos + xlet_height(x) + VSLIDER_EXTRA_Y,
             x, 0);
}
...
static void vslider_draw_move(t_vslider *x, t_glist *glist)
{
    ...
    if(!x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c coords %lxOUT%d %d %d %d %d\n",
             canvas, x, 0,
             xpos, ypos + x->x_gui.x_h - xlet_height(x) + VSLIDER_EXTRA_H,
             xpos + STYLE_XLET_WIDTH, ypos + x->x_gui.x_h + VSLIDER_EXTRA_H);
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c coords %lxIN%d %d %d %d %d\n",
             canvas, x, 0,
             xpos, ypos + VSLIDER_EXTRA_Y,
             xpos + STYLE_XLET_WIDTH, ypos + xlet_height(x) + VSLIDER_EXTRA_Y);
}
...
void vslider_check_height(t_vslider *x, int h)
{
    ...
    if(h < IEM_SL_MINSIZE)
        h = IEM_SL_MINSIZE;
    x->x_gui.x_h = h;
    if(x->x_val > (x->x_gui.x_h*100 - 100) / zoom)
    {
        x->x_pos = (x->x_gui.x_h*100 - 100) / zoom;
        x->x_val = x->x_pos;
    }
    if(x->x_lin0_log1)
        x->x_k = log(x->x_max/x->x_min)/(double)(x->x_gui.x_h / zoom - 1);
    else
        x->x_k = (x->x_max - x->x_min)/(double)(x->x_gui.x_h / zoom - 1);
}
                          ...
                          t_floatarg shift, t_floatarg ctrl, t_floatarg alt)
{
    int zoom = IEMGUI_ZOOM(x);
    if(!x->x_steady)
        x->x_val = (int)(100.0 * (x->x_gui.x_h + (text_ypix(&x->x_gui.x_obj, x->x_gui.x_glist) - ypos)) / zoom);
    if(x->x_val > (100*x->x_gui.x_h / zoom - 100))
        x->x_val = 100*x->x_gui.x_h / zoom - 100;
    if(x->x_val < 0)
        x->x_val = 0;
    x->x_fval = vslider_getfval(x);
    x->x_pos = x->x_val * zoom;
    (*x->x_gui.x_draw)(x, x->x_gui.x_glist, IEM_GUI_DRAW_MODE_UPDATE);
    vslider_bang(x);
